import { BlockUserDto, ProfileDto } from "./dto/profile.dto";
...
export class UserService {
  ...
  async changeProfile(files: ProfileImages, profileDto: ProfileDto) {
    if (files?.image_profile?.length > 0) {
      ...
      profileDto.image_profile = image.path?.slice(7).replace(/\\/g, "/");
    }
    if (files?.bg_image?.length > 0) {
      let [image] = files?.bg_image;
      profileDto.bg_image = image.path?.slice(7).replace(/\\/g, "/");
    }
  ...
  async changeEmail(email: string) {
    ...
    const token = this.tokenService.createEmailTokenToken({ email });
  ...
  async verifyEmail(code: string) {
    ...
    const otp = await this.checkEmailOtp(userId, code);
  ...
  async checkEmailOtp(userId: number, code: string) {
  ...
  async checkPhoneOtp(userId: number, code: string) {
    const otp = await this.otpRepository.findOneBy({ userId });
    if (!otp) throw new BadRequestException(AuthMessage.LoginAgain);
    const now = new Date();
    if (otp.expiresIn < now)
  ...
  async changeUsername(username: string) {
    ...
    if (user && user?.id !== id) {
      throw new ConflictException(ConflictMessage.Usename);
    } else if (user && user?.id == id) {
      return {
        message: PublicMessage.Updated,
      };
  ...
  async blockToggle(blockDto: BlockUserDto) {
